...
- 追加一行到 `index.jsonl`(append, `"a"`),`flush` + `fsync` 保内容落盘;append 之后再 `_fsync_dir(archive_dir)` 让 `index.jsonl` 的目录条目落盘(首次创建该文件时关键——之前的 `_fsync_dir(archive_dir)` 跑在 makedirs 之后、index 文件尚不存在,掉电后首次条目可能未持久化,cc#11/kimi#9)
